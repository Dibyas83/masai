...
"""
Tree  for they allow us to implement a host of algorithms much faster than when using
...
in file systems, graphical user interfaces, databases, Web-sites, and other computer systems

The relationships in a tree are hierarchical, with some objects being “above” and some “below” others.
Actually, the main terminology for tree data structures comes from family trees, with the terms “parent,” “child,”
“ancestor,” and “descendant” being the most common words used to describe rela￾tionships.. With the excep￾tion
of the top element, each element in a tree has a parent element and zero or more children elements

A tree is usually visualized by placing elements inside ovals or rectangles, and by drawing the
connections between parents and children with straight lines. (See Figure 8.2.) We typically call the
...
We see that the internal nodes of the tree are associ￾ated with directories and the leaves are associated
with regular files. In the UNIX and Linux operating systems, the root of the tree is appropriately called
the “root directory,” and is represented by the symbol “/".
...
An edge of tree T is a pair of nodes (u,v) such that u is the parent of v, or vice-versa. A path of
T is a sequence of nodes such that any two consecutive nodes in the sequence form an edge
                                Tree
...
As we did with positional lists in Section 7.4,we define a tree ADT using the concept of a position as
an abstraction for a node of a tree. An element is stored at each position, and positions satisfy
parent-child relationships that define the tree structure. A position object for a tree supports the method:
...
Tree class does not define any internal representation for stor￾ing a tree, and five of the methods given
in that code fragment remain abstract (root, parent, num children, children, and len ); each of these
methods raises a NotImplementedError. (A more formal approach for defining abstract base classes
...
can be implemented based on the former.
 Code Fragment 8.2 provides concrete implementations for methods is root, is leaf, and is empty. In
 Section 8.4, we will explore general algorithms for traversing a tree that can be used to provide
...
The new sibling method is derived from the combination of left, right, and parent. Typ￾ically, we identify 
the sibling of a position p as the “other” child of p’s parent.However, if p is the root, it has no parent, 
and thus no sibling. Also, p may be the only child of its parent, and thus does not have a sibling.
...
We have not yet defined key imple￾mentation details for how a tree will be represented internally, and 
how we can effectively navigate between parents and children. Specifically, a concrete imple￾mentation of 
a tree must provide methods root, parent, num children, children, len , and in the case of BinaryTree, the 
additional accessors left and right
...
We chose not to declare update methods as part of the Tree or BinaryTree ab￾stract base classes for several 
reasons. First, although the principle of encapsula￾tion suggests that the outward behaviors of a class need not 
depend on the internal representation, the efficiency of the operations depends greatly upon the representa￾tion
...
subtrees of leaf position p of
...
To avoid the problem of undesirable update methods being inherited by sub￾classes of LinkedBinaryTree, we 
have chosen an implementation in which none of the above methods are publicly supported. Instead, we provide 
nonpublic ver￾sions of each, for example, providing the underscored delete in lieu of a public delete
...
Memory Optimization: By default, Python instances store their attributes in a dictionary (__dict__). This 
dictionary provides flexibility, allowing new attributes to be added dynamically at runtime. However, a dictionary 
for each instance can consume significant memory, especially when dealing with a large number of instances. 
When __slots__ is defined, Python instead allocates a fixed amount of space for the specified attributes, often 
in a more compact structure like a hidden array of references, thereby reducing memory consumption per instance.

Attribute Access Speed: Storing attributes in a fixed-size structure can also lead to faster attribute lookup 
times compared to dictionary lookups.
Preventing Dynamic Attribute Creation: When __slots__ is defined, instances of that class cannot have new 
attributes added to them at runtime unless those attributes are explicitly listed in __slots__. Attempting to 
add an unlisted attribute will result in an AttributeError. This can be beneficial for enforcing a fixed structure 
and preventing typos or unintended attribute creation.
...
One advantage of an array-based representation of a binary tree is that a posi￾tion p can be represented by 
the single integer f(p), and that position-based meth￾ods such as root, parent, left, and right can be 
implemented using simple arithmetic operations on the number f(p).
...
deleting a node and promoting its child takes O(n) time because it is not just the child that moves locations 
within the array, but all descendants of that child.

When representing a binary tree with a linked structure, each node explicitly main￾tains fields left and right 
as references to individual children. For a general tree,
...
depth(p)                                O(dp +1)
height                                  O(n)
Table 8.2: Running times of the accessor methods of an n-node general tree im￾plemented with a linked structure. 
...
In a preorder traversal of a tree T, the root of T is visited first and then the sub￾trees rooted at its 
children are traversed recursively. If the tree is ordered, then
...
subtrees. The inorder traversal of a binary tree T can be
...
preorder traversal. The only difference is that within the recursive utility for a post￾order we wait to 
yield position p until after we have recursively yield the positions
...
8 path[−1] += 1
9 path.pop( )
Code Fragment 8.24: Efficient recursion for printing an indented and labeled pre￾sentation of a preorder 
traversal.
---------
...
parent as part of the traversal process. A custom solution to the disk space prob￾lem, with each level of 
recursion providing a return value to the (parent) caller, is
...
of recursive tree traversals. Unfortunately, they also show that the specific imple￾mentations of the 
preorder and postorder methods of our Tree class, or the inorder
method of the BinaryTree class, are not general enough to capture the range of
computations we desire.

 In some cases, we need more of a blending of the ap￾proaches, with initial work performed before recurring 
on subtrees, additional work performed after those recursions, and in the case of a binary tree, work performed
between the two possible recursions

Furthermore, in some contexts it was impor￾tant to know the depth of a position, or the complete path from the 
root to that
...
before the subtrees are traversed, and a postvisit hook that is called after the completion of the subtree 
traversals. Our implementation will take the form of an EulerTour class that manages the process, and 
defines trivial defi-nitions for the hooks that do nothing. The traversal can be customized by defining
a subclass of EulerTour and overriding one or both hooks to provide specialized behavior
...
A labeled version of an indented, preorder presentation, akin to Code Frag￾ment 8.24, could be generated by 
the new subclass of EulerTour shown in Code
Fragment 8.30.
...
construct such trees. Each internal node must store a string that defines a binary op￾erator (e.g., + ), 
...
appropriate string. Because it is more efficient to first build a sequence of individ￾ual strings to be 
joined together (see discussion of “Composing Strings” in Sec￾tion 5.4.2), the implementation of str relies
on a nonpublic, recursive method named parenthesize recur that appends a series of strings to a list
...
return x ◦ y
Code Fragment 8.36: Algorithm evaluate recur for evaluating the expression rep￾resented by a subtree of an 
arithmetic expression tree rooted at position p.
